...
interface DbSchema {
  ...
  processed_rewards: Record<string, string>; // sessionId -> processedAt
  notification_status: Record<string, { sent: number; sentAt: string }>;
}
...
function saveDb() {
  try {
    ...
    fs.writeFileSync(tempPath, JSON.stringify(memoryDb, null, 2), 'utf-8');
...
export function getOperationsSince(sinceId: number): Operation[] {
  loadDb();
  return memoryDb.operations.filter(op => op.id !== undefined && op.id > sinceId);
}

// Process rewards exactly once
export function processRewardsForSession(sessionId: string, coins: number): boolean {
  loadDb();
...
export function getNotificationStatus(sessionId: string): { sent: number; sentAt: string } | null {
  loadDb();
  const status = memoryDb.notification_status[sessionId];
  return status || null;
}

export function setNotificationStatus(sessionId: string, sent: number): void {
  loadDb();
